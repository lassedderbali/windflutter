...
class WindowProvider extends ChangeNotifier {
  ...
  String _activeWindowId = '1';
  String _viewMode = 'single'; // single, multiple, combined
  ...
  WindowProject? get activeWindow => 
      _windows.firstWhere((w) => w.id == _activeWindowId, orElse: () => _windows.first);
  ...
  void removeWindow(String id) {
    ...
    if (_activeWindowId == id) {
      _activeWindowId = _windows.first.id;
    }
    notifyListeners();
  }
  ...
  void updateWindowSpecs(WindowSpecs specs) {
    final index = _windows.indexWhere((w) => w.id == _activeWindowId);
    if (index != -1) {
